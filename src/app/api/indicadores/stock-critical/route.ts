...
import { getStockCritical } from '@/lib/indicadores/mantenimientos'
...
export async function GET(request: NextRequest) {
  ...
  try {
    const params = parseRangeWithLimitParams(searchParams, 8)
    const cacheControl = parseCacheControlParams(searchParams)
    const options = { ...cacheControl, limit: params.limit }
    const data = await getStockCritical(params.from, params.to, options)
    return NextResponse.json(data, { status: 200 })
  } catch (error) {
    if (error instanceof ZodError) {
      return NextResponse.json({ error: 'Parámetros inválidos', detalles: error.flatten() }, { status: 400 })
    }
    console.error('Error consultando stock crítico', error)
    return NextResponse.json({ error: 'No se pudo calcular la disponibilidad de repuestos críticos.' }, { status: 500 })
  }
}
