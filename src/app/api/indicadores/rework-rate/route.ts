...
import { getReworkRate } from '@/lib/indicadores/mantenimientos'
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await getReworkRate(params.from, params.to, options)
    return NextResponse.json(data, { status: 200 })
  } catch (error) {
    if (error instanceof ZodError) {
      return NextResponse.json({ error: 'Parámetros inválidos', detalles: error.flatten() }, { status: 400 })
    }
    console.error('Error consultando indicador de retrabajo', error)
    return NextResponse.json({ error: 'No se pudo calcular la tasa de retrabajo.' }, { status: 500 })
  }
}
