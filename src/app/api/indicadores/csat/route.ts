...
import { getCsat } from '@/lib/indicadores/mantenimientos'
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await getCsat(range.from, range.to, cacheControl)
    return NextResponse.json(data, { status: 200 })
  } catch (error) {
    if (error instanceof ZodError) {
      return NextResponse.json({ error: 'Parámetros inválidos', detalles: error.flatten() }, { status: 400 })
    }
    console.error('Error consultando indicador CSAT', error)
    return NextResponse.json({ error: 'No se pudo calcular el indicador de satisfacción.' }, { status: 500 })
  }
}
