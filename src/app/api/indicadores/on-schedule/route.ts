...
import { getOnSchedule } from '@/lib/indicadores/mantenimientos'
import { parseCacheControlParams, parseOnScheduleParams } from '@/lib/indicadores/params'
...
export async function GET(request: NextRequest) {
  ...
  try {
    const params = parseOnScheduleParams(searchParams)
    const cacheControl = parseCacheControlParams(searchParams)
    const data = await getOnSchedule(params.from, params.to, {
    ...
    console.error('Error consultando indicador on-schedule', error)
    return NextResponse.json({ error: 'No se pudo calcular el indicador de cumplimiento.' }, { status: 500 })
  }
}
