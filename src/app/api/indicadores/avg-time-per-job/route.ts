...
import { getAvgTimePerJob } from '@/lib/indicadores/mantenimientos'
import { parseCacheControlParams, parseRangeWithLimitParams } from '@/lib/indicadores/params'
...
export async function GET(request: NextRequest) {
  ...
  try {
    const params = parseRangeWithLimitParams(searchParams)
    const cacheControl = parseCacheControlParams(searchParams)
    const options = { ...cacheControl, limit: params.limit }
    const data = await getAvgTimePerJob(params.from, params.to, options)
    return NextResponse.json(data, { status: 200 })
  } catch (error) {
    if (error instanceof ZodError) {
      return NextResponse.json({ error: 'Parámetros inválidos', detalles: error.flatten() }, { status: 400 })
    }
    console.error('Error consultando tiempo promedio por servicio', error)
    return NextResponse.json({ error: 'No se pudo calcular el tiempo promedio por servicio.' }, { status: 500 })
  }
}
