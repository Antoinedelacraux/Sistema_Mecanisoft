...
import { getRescheduleRate } from '@/lib/indicadores/mantenimientos'
import { parseCacheControlParams, parseRangeParams } from '@/lib/indicadores/params'
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await getRescheduleRate(range.from, range.to, cacheControl)
    return NextResponse.json(data, { status: 200 })
  } catch (error) {
    if (error instanceof ZodError) {
      return NextResponse.json({ error: 'Parámetros inválidos', detalles: error.flatten() }, { status: 400 })
    }
    console.error('Error consultando indicador de reprogramación', error)
    return NextResponse.json({ error: 'No se pudo calcular la tasa de reprogramación.' }, { status: 500 })
  }
}
