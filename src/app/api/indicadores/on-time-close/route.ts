...
import { getOnTimeClose } from '@/lib/indicadores/mantenimientos'
import { parseCacheControlParams, parseOnTimeCloseParams, parseSlaMappingParam } from '@/lib/indicadores/params'
...
export async function GET(request: NextRequest) {
  ...
  try {
  const params = parseOnTimeCloseParams(searchParams)
  const cacheControl = parseCacheControlParams(searchParams)
  const slaMapping = parseSlaMappingParam(searchParams)
  const options = slaMapping ? { ...cacheControl, slaMapping } : cacheControl
  const data = await getOnTimeClose(params.from, params.to, options)
    return NextResponse.json(data, { status: 200 })
  } catch (error) {
    if (error instanceof ZodError) {
      return NextResponse.json({ error: 'Parámetros inválidos', detalles: error.flatten() }, { status: 400 })
    }
    console.error('Error consultando indicador on-time close', error)
    return NextResponse.json({ error: 'No se pudo calcular el indicador de cierre a tiempo.' }, { status: 500 })
  }
}
