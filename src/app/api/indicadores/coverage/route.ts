...
import { getCoverage } from '@/lib/indicadores/mantenimientos'
...
export async function GET(request: NextRequest) {
  ...
  const data = await getCoverage(range.from, range.to, cacheControl)
    return NextResponse.json(data, { status: 200 })
  } catch (error) {
    if (error instanceof ZodError) {
      return NextResponse.json({ error: 'Parámetros inválidos', detalles: error.flatten() }, { status: 400 })
    }
    console.error('Error consultando cobertura de mantenimientos', error)
